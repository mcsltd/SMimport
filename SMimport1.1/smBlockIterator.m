% smBlockIterator - Internal class of SMimport for decoding SM-file records.
...
classdef smBlockIterator
    ...
    %   crc - crc of block
    %
    % For iteration call next(), which returns a block
    % When next() returns empty array, it means EOF reached.
    % It's possible to extract block  directly by it's id later
    % with extract_block() funciton

    properties (Constant = true, Hidden = true)
        BLOCK_PREFIX = hex2dec('11FF');
        BLOCK_HEADER_SIZE = 7;
        BLOCK_SUFFIX_SIZE = 4;
        crc32_table = uint32(hex2dec([
